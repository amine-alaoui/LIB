...
var macPayloadRegistry = map[bool]map[CID]macPayloadInfo{
	false: map[CID]macPayloadInfo{
		...
		LinkADRReq:          {6, func() MACCommandPayload { return &LinkADRReqPayload{} }},
...
func (p *LinkADRReqPayload) UnmarshalBinary(data []byte) error {
	...
	// Faire Attention si la valeur du bandwith est égal à 500
	if data[4] == 125 || data[4] == 250 {
		p.Bandwidth = data[4]
	} else {
		p.Bandwidth = 125
	}
	if data[5] == 1 || data[5] == 2 || data[5] == 3 || data[5] == 4 || data[5] == 5 {
		p.CodeRate = data[5]
	} else {
		p.CodeRate = 1
	}
...
func decodeDataPayloadToMACCommands(uplink bool, payloads []Payload) ([]Payload, error) {
	...
	for i := 0; i < len(dataPL.Bytes); i++ {
		...
		//fmt.Printf("*******payload getSize: %d \n", plLen)
		//fmt.Printf("*******datapayload size : %d \n", len(dataPL.Bytes[i:]))
		//fmt.Printf("*******datapayload size %d doit etre >=  payload getsize+1 %d \n", len(dataPL.Bytes[i:]), plLen+1)
		...
		fmt.Printf("\n*******pMAC COMMANDs before unMarshal %+v \n", dataPL.Bytes[i:i+1+plLen])
		fmt.Printf("\n*******pMAC COMMANDs %+v \n", mc.Payload)
